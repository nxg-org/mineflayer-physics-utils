...
export type EBounceBot = PhysicsBot & {
...
export function findInventoryItem(bot: Bot, itemName: string) {
  return bot.inventory.items().find((item) => item.name === itemName) ?? null;
}

export function findEquippedItem(bot: Bot, destination: "hand" | "torso" | "off-hand", itemName: string) {
  const slot = (() => {
    switch (destination) {
      case "hand":
        return 0;
...
export function findElytra(bot: Bot) {
  return findInventoryItem(bot, "elytra") ?? findEquippedItem(bot, "torso", "elytra");
}

export function findFireworkRocket(bot: Bot) {
  return (
...
export function isUsableElytra(item: ReturnType<typeof findElytra>) {
...
export async function ensureBounceLoadout(bot: Bot) {
...
export type BounceVector = { x: number; y: number; z: number };
...
export const DEFAULT_EBOUNCE_OPTIONS: EBounceOptions = {
...
export class MineflayerEBouncePort implements EBouncePort {
  constructor(
...
export class EBounceController {
  ...
  public isBouncing() {
    return this.currentState === FlightState.BOUNCING;
  }
...
export function registerEBounceLogging(bot: EBounceBot, controller: EBounceController, enabled: boolean = true) {
  if (!enabled) return;
